...
public class FrameAnimView extends RecyclingImageView {
    ...
    private void init(AttributeSet attrs) {
        if (attrs != null) {
            TypedArray typedArray = getContext().obtainStyledAttributes(attrs, R.styleable.conmonmodule_frameanimview);
            int resId = typedArray.getResourceId(R.styleable.conmonmodule_frameanimview_conmonmodule_animsrc, 0);
            if (resId != 0) {
                loadFromXml(resId);
            }
            typedArray.recycle();
